...
class Data_loader:
    ...
    def __init__(self, path='maps/COM_PowerSpect_CMB-TT-full_R3.01.txt', lmax=200, n_xvals=1800, time_it=False):
        ...
        try:
            ...
            logger.error(f"Error loading data file {self.path}: {e}")
            return
    ...
    def _load_matrix(self, a, b):
        ...
        try:
            ...
            # Attempt to compute Tmn and save it if missing
            try:
                from .s12 import Tmn
                logger.info(f"Matrix file not found: {matrix_path}. Computing Tmn and saving...")
                Tmn(len(self.D_ell), theta_upper, theta_lower, a, b)
                return np.load(matrix_path)
            except Exception as e:
                logger.exception(f"Failed to compute Tmn for {matrix_path}: {e}")
                raise
    @time_execution
    ...
    def load_run(self, run_dir):
        """
        Load results from a previous run folder created by the main workflow.
        ...
        Returns:
            dict: {'simulation': {matrices and scalars}, 'theory': {model_name: {}}}
        """
        result = {'simulation': None, 'theory': {}}
        sim_dir = os.path.join(run_dir, 'simulation')
        if os.path.exists(sim_dir):
            try:
                D_sim = np.load(os.path.join(sim_dir, 'D_ell.npy'))
                C_sim = np.load(os.path.join(sim_dir, 'Corr.npy'))
            except Exception:
                D_sim = None
                C_sim = None
            cols = []
            cols_path = os.path.join(sim_dir, 'columns.txt')
            if os.path.exists(cols_path):
                with open(cols_path, 'r') as cf:
                    cols = [line.strip() for line in cf if line.strip()]
            data = {}
            if os.path.exists(os.path.join(sim_dir, 'S_statistics.txt')) and cols:
                S_mat = np.loadtxt(os.path.join(sim_dir, 'S_statistics.txt'))
                if S_mat.ndim == 1:
                    S_mat = S_mat[:, None]
                s_cols = [c for c in cols if c.startswith('s12_')]
                for i, c in enumerate(s_cols):
                    data[c] = S_mat[:, i]
            if os.path.exists(os.path.join(sim_dir, 'xiv_statistic.txt')) and cols:
                xiv_mat = np.loadtxt(os.path.join(sim_dir, 'xiv_statistic.txt'))
                if xiv_mat.ndim == 1:
                    xiv_mat = xiv_mat[:, None]
                xiv_cols = [c for c in cols if c.startswith('xiv_')]
                for i, c in enumerate(xiv_cols):
                    data[c] = xiv_mat[:, i]
            if os.path.exists(os.path.join(sim_dir, 'C180_statistics.txt')):
                data['C180'] = np.loadtxt(os.path.join(sim_dir, 'C180_statistics.txt'))
            if data:
                df = pd.DataFrame(data)
            else:
                df = pd.DataFrame()
            if D_sim is not None:
                df['D_ell'] = [row for row in D_sim]
            if C_sim is not None:
                df['Cor'] = [row for row in C_sim]
            result['simulation'] = {'D_ell': D_sim, 'Corr': C_sim, 'dataframe': df, 'columns': cols}

        # Theory models
        theory_base = os.path.join(run_dir, 'theory')
        if os.path.exists(theory_base):
            for model in sorted(os.listdir(theory_base)):
                model_dir = os.path.join(theory_base, model)
                if not os.path.isdir(model_dir):
                    continue
                Dp = os.path.join(model_dir, 'D_ell.npy')
                Cp = os.path.join(model_dir, 'Corr.npy')
                cols_p = os.path.join(model_dir, 'columns.txt')
                try:
                    D_mat = np.load(Dp) if os.path.exists(Dp) else None
                    C_mat = np.load(Cp) if os.path.exists(Cp) else None
                except Exception:
                    D_mat = None
                    C_mat = None
                cols_list = []
                if os.path.exists(cols_p):
                    with open(cols_p, 'r') as cf:
                        cols_list = [line.strip() for line in cf if line.strip()]
                data = {}
                ...
                if os.path.exists(S_path) and cols_list:
                    S_mat = np.loadtxt(S_path)
                    if S_mat.ndim == 1:
                        S_mat = S_mat[:, None]
                    s_cols = [c for c in cols_list if c.startswith('s12_')]
                    for i, c in enumerate(s_cols):
                        data[c] = S_mat[:, i]
                if os.path.exists(xiv_path) and cols_list:
                    xiv_mat = np.loadtxt(xiv_path)
                    if xiv_mat.ndim == 1:
                        xiv_mat = xiv_mat[:, None]
                    xiv_cols = [c for c in cols_list if c.startswith('xiv_')]
                    for i, c in enumerate(xiv_cols):
                        data[c] = xiv_mat[:, i]
                if os.path.exists(C180_path):
                    data['C180'] = np.loadtxt(C180_path)
                if data:
                    df = pd.DataFrame(data)
                else:
                    df = pd.DataFrame()
                if D_mat is not None:
                    df['D_ell'] = [row for row in D_mat]
                if C_mat is not None:
                    df['Cor'] = [row for row in C_mat]
                result['theory'][model] = {'D_ell': D_mat, 'Corr': C_mat, 'dataframe': df, 'columns': cols_list}
