...
- The main driver is `main.py`, which reads a YAML configuration (creates a default if missing) and decides an output run directory using a short hash of the configuration. The run name encodes lmax, n_samples and mode and the code will reuse an existing run directory if only the `roots` parameter changed.

- Modes supported: `bestfit`, `mcmc`, `simulation`, and `all`.
  - `bestfit`: reads a best-fit theory spectrum and runs the Planck-style pipeline many times to build an ensemble of recovered spectra and scalar statistics.
  - `mcmc`: loads GetDist MCMC chains, computes theoretical spectra per sample (CAMB) and runs the pipeline per sample to obtain derived statistics; saves results into `theory_mcmc/`.
  - `simulation`: generates Monte Carlo realizations from the experimental D_ell and errors and computes the distribution of statistics; saved under `simulation/`.
  - `all`: runs bestfit + mcmc + simulation and saves separate `theory_bestfit/` and `theory_mcmc/` directories.

- After computation the script optionally runs plotting (`plots.CorrelationPlots`) and table generation (`generate_result_tables.generate_all_tables`). Plots are written to `images_{mode}/` subdirectories; scalar statistics (S12, xiv, C180) and arrays (D_ell.npy, Corr.npy) are saved under `theory_*` or `simulation/`.
